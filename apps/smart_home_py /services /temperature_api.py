    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            while True:
                ...
                cur.execute("SELECT sensor_id FROM sensors_py WHERE sensor_id = %s", (new_id,))
                if not cur.fetchone():
                    return new_id
    finally:
        conn.close()
...
def create_sensor(sensor: SensorCreate):
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                """
                INSERT INTO sensors_py (sensor_id, name, type, location, value, unit, status, last_updated, created_at)
...
def get_all_sensors():
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute("SELECT * FROM sensors_py")
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            if location and not sensor_id:
                ...
                cur.execute("SELECT * FROM sensors_py WHERE location = %s", (location.strip(),))
                sensor = cur.fetchone()
                if not sensor:
                ...
                cur.execute("SELECT * FROM sensors_py WHERE sensor_id = %s", (sensor_id,))
                sensor = cur.fetchone()
                if not sensor:
                    raise HTTPException(status_code=400, detail="Invalid sensor_id")
            else:
                # Both provided, verify they match
                cur.execute("SELECT * FROM sensors_py WHERE sensor_id = %s AND location = %s",
